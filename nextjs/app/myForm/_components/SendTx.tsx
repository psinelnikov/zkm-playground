  ...
  const clickSubmit = async (e: any) => {
    ...
    try {
      ...
      writeContract({
        ...
        args: [address, 1000000000000000000, uint256input, proofData, commitments],
      });
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="flex flex-col items-center mt-2">
      ...
      {hash && (
        <div>
          Transaction Hash:{" "}
          <a target="_blank" href={"https://sepolia-explorer.metisdevops.link/tx/" + hash}>
            {hash}
          </a>
        </div>
      )}
