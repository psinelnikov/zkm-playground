...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input: ProverInput = match args[1].as_str() {
        ...
        "sigverify-go" => set_sigverify_go_input(seg_size2, execute_only2, elf_path, args_parameter)
            .expect("set sigverify-go input error"),
        "emailverify-go" => set_emailverify_go_input(seg_size2, execute_only2, elf_path, args_parameter)
            .expect("set emailverify-go input error"),
            ...
            if !execute_only2 {
                ...
                match args[1].as_str() {
                    ...
                    "sigverify-go" => {
                        ...
                        log::info!("Executed the guest program successfully.");
                        let ret_data: SignedMessage =
                            bincode::deserialize_from(prover_result.output_stream.as_slice())
                                .expect("deserialization failed");
                        log::info!("ret_data: {:?}", ret_data);
                    }
...
fn set_sigverify_go_input(
    seg_size_u: u32,
    execute_only_b: bool,
    elf_path: String,
    args: String,
) -> anyhow::Result<ProverInput> {
    // assume the  arg[0] is the message, arg[1] is the address, and arg[2] is the signature.
    let args: Vec<&str> = args.split_whitespace().collect();
    assert_eq!(args.len(), 3);
    let mut signed_message = SignedMessage::new();
    // Fill in the input data
    signed_message.message = args[0].to_string();
    signed_message.signature = args[1].to_string();
    signed_message.address = args[2].to_string();
    
    let mut buf = Vec::new();
    bincode::serialize_into(&mut buf, &signed_message).expect("serialization failed");
    let input = ProverInput {
        elf: read(elf_path).unwrap(),
    ...
    log::info!(
        "sigverify-go, bincode(public_input): {:?} ",
        &input.public_inputstream
    );
    Ok(input)
}
...
fn set_emailverify_go_input(
    seg_size_u: u32,
    execute_only_b: bool,
    elf_path: String,
    args: String,
) -> anyhow::Result<ProverInput> {
    // Split the input string by whitespace to get individual emails
    let args: Vec<&str> = args.split_whitespace().collect();
    assert_eq!(args.len(), 3);

    let user_data = UserData {
        email: args[0].to_string(),
        eth_address: args[1].to_string(),
        number: args[2].to_string(),
    };
    
    let mut buf = Vec::new();
    bincode::serialize_into(&mut buf, &user_data).expect("serialization failed");
    let input = ProverInput {
        elf: read(elf_path).unwrap(),
        public_inputstream: buf,
        private_inputstream: "".into(), //the private input is empty
    ...
    log::info!(
        "emailverify-go, bincode(public_input): {:?} ",
        &input.public_inputstream
    );
    Ok(input)
}
